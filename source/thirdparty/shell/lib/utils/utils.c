...
int r32(int argc, char *argv[]) {
  if (argc < 2) {
    printf_esh("Usage: %s <address> (in hex)\n", argv[0]);
    return -1;
  }
  ...
  printf_esh("0x%x: 0x%x\n", addr, data);
  return 0;
}

int w32(int argc, char *argv[]) {
  if (argc < 3) {
    printf_esh("Usage: %s <address> (in hex) <value> (in hex)\n", argv[0]);
    return -1;
  }
  uint32_t addr = atoh(argv[1]);
...
int read_mem(int argc, char *argv[]) {
  if (argc < 3) {
    printf_esh("Usage: %s <address> (in hex) <num_words> (in decimal)\n", argv[0]);
    return -1;
  }
...
int w32_bit(int argc, char *argv[])
{
  ...
  if (bit_loc > 31) {
    printf_esh("Error: Invalid bit location argument\n");
    goto usage;
  }

  if (bit_val != 0 && bit_val != 1) {
    printf_esh("Error: Invalid bit value argument\n");
    goto usage;
  }
  ...
  return 0;
usage:
  printf_esh("Usage: %s <address_in_hex(32-bits)> <bit_location_in_decimal(0 to 31)> <bit_value(0 or 1)>\n", argv[0]);
  return -1;
}
...
int r32_bit(int argc, char *argv[])
{
  ...
  printf_esh("Bit %u @ 0x%08x: %u\n", bit_loc, addr, bit_val);
  return 0;

usage:
  printf_esh("Usage: %s <address_in_hex(32-bits)> <bit_location_in_decimal(0 to 31)\n", argv[0]);
  return -1;
}
