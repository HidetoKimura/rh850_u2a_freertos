...
#include "lib\a2i\a2i.h"
#include "lib\printf\printf.h"
#include "lib\regs\regs.h"
#include "string.h"
 ...
 */
  ...
  #define ADD_CMD(_name, _help_string, _function) \
    ...
    #define CMD_END() \
    __attribute__((section(".cmd_end"))) \
    const volatile  cmd_t cmd_end_ = { NULL, NULL, NULL}
#endif


#if defined (__ICCRH850__)
  #define ADD_CMD(_name, _help_string, _function) \
    __root const __attribute__((section(".cmd_list"))) \
    cmd_t _var##_function = {#_name, _help_string, &_function}

  #define AUTO_CMD(_name, _help_string, _function) \
    __root const __attribute__((section(".auto_list"))) \
    cmd_t _var##_function = {#_name, _help_string, &_function}
    
  #define CMD_END() \
   __attribute__((section(".cmd_end")))   \
  const __root  cmd_t cmd_end_ = { NULL, NULL, NULL}
#endif
  ...
  #define ADD_CMD(_name, _help_string, _function)     \
  const char string_##_name[] = _help_string;\
  const char name_##_name[] = #_name; \
  _Pragma("section const \".cmd_list\"") \
  const cmd_t _var##_function = {name_##_name, string_##_name, &_function} \
  _Pragma("section") 

  #define AUTO_CMD(_name, _help_string, _function)  \
  const char string_##_name[] = _help_string;\
  const char name_##_name[] = #_name; \
  _Pragma("section const \".auto_list\"") \
  const cmd_t _var##_function = {name_##_name, string_##_name, &_function} \
  _Pragma("section") 
  
  #define CMD_END() \
  _Pragma("section const \".cmd_end\"") \
  const cmd_t cmd_end_ = { NULL, NULL, NULL}; \
  _Pragma("section") 
