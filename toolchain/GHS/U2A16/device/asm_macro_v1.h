...
#if !defined(__RH850G4KH__) &!defined(__RH850G4MH__) & defined(__E2XFCC1)
...
#define SHIFTCORE       shr	16, r10
    ...
    .set PORTP0PSR33 0xff610844
    .set BR0BASE     0xfffb8000 
    .set BR0INIT     0x000          -- cleared to zero after reset
    .set BR0EN       0x004          -- cleared to zero after reset
    ...
    -- Only the first PE (0 or 1) in the system may boot all other cores
    -- Please note, this is valid for systems without emulator attached!
    -- Using E1 emulator will always activate ALL cores
    .macro BOOT_ALL_PE pe
    GETPEID
    cmp     pe, r10
    bne     1f
...
1:    
    ...
    .macro INITAL_BOOT_BARRIER
    GETPEID
    cmp     r0, r10     -- r10 is in a range between 0 and 5
    be      2f          -- if (PE == 0) leave function
...
--  st.w      r1, 0[r12]
 ...
 2:
    ...
    -- occupies r10, skips exectution to 1f
    .macro skip21f pe
    GETPEID
    cmp     pe, r10
    bne     1f
...
-------------  if the core executing the code is PE0
-------------------------------------------------------------------------------
	.macro jmpIfPE0	addr
	GETPEID
	cmp	PE0, r10
...
-------------------------------------------------------------------------------
-------------  Jump to a target address (label) specified as a parameter
-------------  if the core executing the code is PE1
-------------------------------------------------------------------------------
	.macro jmpIfPE1	addr
	GETPEID
	cmp	PE1, r10
...
#ifdef __RH850_additional_sda_reg
 #if __RH850_additional_sda_reg>0
