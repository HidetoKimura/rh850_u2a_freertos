...
void (*__write_char__)(char c);
  ...
  #pragma section=".auto_list"
  #pragma section=".cmd_list"
  #define __cmd_list_start __section_begin(".cmd_list")
  #define __auto_list_start __section_begin(".auto_list")
  cmd_t *volatile table=  (cmd_t *)__cmd_list_start;
  cmd_t *volatile auto_load= (cmd_t *)__auto_list_start;
#endif

#if defined (__ghs__)
  extern cmd_t __cmd_list_start;
  extern cmd_t __auto_list_start;
  cmd_t *volatile table = &__cmd_list_start; 
  cmd_t *volatile auto_load = &__auto_list_start; 
#endif

#if defined (__CCRH__)
  extern char _S_cmd_list_const; 
  extern char _S_auto_list_const; 
  cmd_t *volatile table=  (cmd_t *)&_S_cmd_list_const;
  cmd_t *volatile auto_load= (cmd_t *)&_S_auto_list_const;
#endif
...
static void execute(int argc, char **argv) {
  ...
  if (match_found == FALSE) {
    printf_esh("\"%s\": command not found. Use \"help\" to list all command.\n",
           argv[0]);
    __cmd_exec_status = -1;
  }
}
...
#if defined (__CCRH__)
#define COMPILERNAME "CCRH" 
#elif defined (__ghs__)
#define COMPILERNAME "GHS" 
#elif defined (__ICCRH850__)
#define COMPILERNAME "IAR" 
#endif


static int build_info(int argc, char **argv) {
  printf_esh("Build: U2A Starterkit Software - " COMPILERNAME " - " __DATE__ " - " __TIME__ "\r\n");
  return 0;
}
...
int help(int argc, char **argv) {
  ...
  if (argc > 1 && (strcmp(argv[1], "-l")==0)) {
    ...
    printf_esh("use: help -l for list only.\n\n");
  }

  while (table[i].command_name != NULL) {
    printf_esh(table[i].command_name);

    if (verbose) {
      printf_esh("\n\t");
      printf_esh(table[i].command_help);
    }
